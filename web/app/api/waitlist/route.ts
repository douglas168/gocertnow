...
export async function POST(req: NextRequest) {
  ...
  await resend.emails.send({
    from: "LevelCert <hello@levelcert.com>",
    to: email,
    subject: isZh ? "您已成功加入 LevelCert 候補名單！" : "You're on the LevelCert waitlist!",
    html: isZh
      ? `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">
          <h2 style="color:#51a2ff;margin-bottom:8px;">您已成功加入！🎉</h2>
          <p>感謝您加入 <strong>LevelCert</strong> 候補名單。</p>
          ...
          <p style="color:#7B9CC4;font-size:13px;margin-top:24px;">— LevelCert 團隊</p>
        </div>
      `
      : `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">
          <h2 style="color:#51a2ff;margin-bottom:8px;">You're on the list! 🎉</h2>
          <p>Thanks for joining the <strong>LevelCert</strong> waitlist.</p>
          ...
          <p style="color:#7B9CC4;font-size:13px;margin-top:24px;">— The LevelCert Team</p>
        </div>
      `,
  });
