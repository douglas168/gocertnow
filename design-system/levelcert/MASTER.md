...
3. **Bilingual by design** — Traditional Chinese (繁體中文) for Taiwan IPAS pilot, English for future international certs. Every layout must handle both.
4. **Mobile-first study behavior** — Students study on commute and lunch breaks. Touch targets, readable text, and fast load times are non-negotiable.
...
Dark mode is the default theme. This matches the gaming platforms (Discord, Twitch, Steam) and apps (Line) that the 20-35 target audience uses daily. Indigo primary signals modern tech competence; emerald CTA means "go, grow, achieve."
...
### Color Palette — Light Mode (Optional Toggle)

Available as a user preference toggle. Some users may prefer light mode for extended reading sessions.

| Role | Hex | Tailwind | CSS Variable | Usage |
|------|-----|----------|--------------|-------|
| Primary | `#4F46E5` | `indigo-600` | `--color-primary` | Same primary across modes |
| Secondary | `#6366F1` | `indigo-500` | `--color-secondary` | Slightly darker for light bg contrast |
| CTA / Accent | `#059669` | `emerald-600` | `--color-cta` | Slightly darker for light bg contrast |
| Background | `#F8FAFC` | `slate-50` | `--color-background` | Page background |
| Surface | `#FFFFFF` | `white` | `--color-surface` | Cards, modals, input backgrounds |
| Text | `#1E293B` | `slate-800` | `--color-text` | Body text |
| Text Muted | `#475569` | `slate-600` | `--color-text-muted` | Secondary text, captions, timestamps |
| Border | `#E2E8F0` | `slate-200` | `--color-border` | Card borders, dividers, input borders |
...
**Note:** RPG accents use the `-400` weight (lighter) to maintain vibrancy on dark backgrounds. On light mode, they auto-shift to `-500` for sufficient contrast.
...
**CJK-Specific Rules:**
- Body line height must be >= 1.7 for Traditional Chinese readability (1.75 recommended)
- Test all layouts with both zh and en text — Chinese strings are typically shorter but taller
...
Use CSS variables in `:root` and `.dark` — never hardcode hex values in components.
...
.light {
  --primary: 239 84% 67%;       /* #4F46E5 indigo-600 */
  --secondary: 239 84% 67%;     /* #6366F1 indigo-500 */
  --accent: 161 93% 30%;        /* #059669 emerald-600 */
  --background: 210 40% 98%;    /* #F8FAFC slate-50 */
  --foreground: 215 28% 17%;    /* #1E293B slate-800 */
  --muted-foreground: 215 16% 37%; /* #475569 slate-600 */
...
| 6. Pricing | Single tier, clear price, 3-month access, pass guarantee callout | Orange CTA button |
...
- Layout must not break when text length differs between zh/en
...
### Dark/Light Mode
- [ ] Dark mode (default) text contrast >= 4.5:1
- [ ] Light mode toggle text contrast >= 4.5:1
- [ ] Borders visible in both modes
- [ ] RPG accents use `-400` weight on dark, `-500` on light
